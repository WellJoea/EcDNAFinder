...
def Args():
    ...
    P_update.add_argument("-rp", "--regionpre", type=str, default='All.circle.region',
                    help="out prefix of regioin out put.")
    P_Update = subparsers.add_parser('Update',conflict_handler='resolve',
                    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                    parents=[P_Common, P_fetch, P_search, P_merge, P_update],
    ...
    P_filter.add_argument("-dn", "--minsupportidnum", type=int, default=1,
    ...
    P_filter.add_argument("-bm", "--breakpiontnum", type=int, default=2,
                    help="keep the links with the threshlod of max breakpoint number in all samples. [Default: %(default)s]")
    P_Filter = subparsers.add_parser('Filter',conflict_handler='resolve',
                    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                    parents=[P_Common, P_merge, P_update, P_filter],
                    help='filter links from bed file.')

    P_cvisual = subparsers.add_parser('CVisual', conflict_handler='resolve', add_help=False)
    P_cvisual.add_argument("-ci", "--circisin", type=str,
                    help="the circos input file. [Deflaut: UpFilterTRF].")
    P_cvisual.add_argument("-co", "--circisout", type=str,
                    help="the circos output directory. [Deflaut: Update dir].")
    P_cvisual.add_argument("-cs", "--circissw", type=str,
                    default='/share/home/share/software/circos-0.69-9/bin/circos',
                    help="the circos software. [Default: %(default)s].")
    P_cvisual.add_argument("-hd", "--cirhead", type=str, default='All.circle.plot',
                    help="the circos output prefix name. [Default: %(default)s].")
    P_CVisual = subparsers.add_parser('CVisual',conflict_handler='resolve',
                    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                    parents=[P_Common, P_merge, P_filter, P_cvisual],
    ...
    P_Autopipe = subparsers.add_parser('Auto', conflict_handler='resolve', prefix_chars='-+',
                    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                    parents=[P_Common, P_fetch, P_search, P_merge, P_region, P_filter, P_check],
                    help='the auto-processing for all.')
    P_Autopipe.add_argument("+P", "++pipeline", nargs='+', default=['Fetch', 'Search', 'Merge', 'Update', 'Filter'],
                    help="the auto-processing: Fetch, Search, Merge, Filter, Update. [Default: %(default)s]")
