...
class Utilities():
    ...
    def annogene(self, _inbed, minoverA=30, minoverB=30, 
        ...
        Fcol = ['#chrom', 'start', 'end', 'length', 'support_sum',
                'head_dist', 'tail_dist', 'gene_name', 'gene_id', 'gene_biotype']
...
class SearchType():
    ...
    def mergeNeighb(self, _inbed, maxdistance=200, maxsimilar=50, maxreg=True, oriant=True):
    ...
    def EachEcDNA(self):
        ...
        if not self.inbed.empty:
            ...
            kmerge = pd.DataFrame()
            print('cannot find any circle retion singal: ' + self._inbed)
    ...
    def mergeSamples(self, _inbed):
        ...
        UpLinks = Parallel( n_jobs=-1,  backend='loky')( delayed( self.linkParall )(_g) for _l, _g in uplinks.groupby(by='links_sorted', sort=False) )
        UpLinks = pd.concat(UpLinks, axis=0, sort=False)
...
class ecDNA():
    ...
    def __init__(self, arg, log,  *array, **dicts):
        ...
        os.environ['NUMEXPR_MAX_THREADS'] = str(int(CORES))
        os.environ['PATH'] += ':' + self.arg.bedtools
        #bt.helpers.set_bedtools_path(elf.arg.bedtools)
        import importlib
        importlib.reload(bt)

        self.arg.BamDir     = '%s/%s'%(self.arg.outdir, self.arg.infetch)
    ...
    def allwork(self, _INdf, Chrom=False):
        AllMerge = []
        ...
        if AllMerge:
            ...
            SearchType(AllMerge, self.arg.AllTypeDir  + '/All.basedon.Keep', Chrom=Chrom).AllEcDNA()
        else:
    ...
    def Pipeline(self):
        ...
        if self.arg.commands in ['Region', 'Auto']:
            #self.allwork(INdf, Chrom=False)
            self.allwork(INdf, Chrom=True)
...
def Args():
    ...
    P_Common.add_argument("-n", "--njob",type=str,default=5,
...
#OUTdir='/data/zhouwei/01Projects/03ecDNA/V2'
